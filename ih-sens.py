...
6. Normalized forward sensitivity indices of i_h at the endemic equilibrium
   to all 17 parameters, computed via the implicit function theorem (Az=b).
...
def rhs_scaled(t, y, p: Params):
    ...
    contact = (p.sigma_v * p.sigma_h) / denom
...
# -----------------------------------------------------------------------------
# 4b.  Sensitivity indices of x_ee (specifically i_h)
# -----------------------------------------------------------------------------


def equilibrium_equations(y, p: Params):
    """Returns the RHS of the scaled ODE system at steady state (should be ~0)."""
    return rhs_scaled(0, y, p)


def sensitivity_indices_xee(p: Params, y_ee, rel_step: float = 1e-5) -> dict[str, dict[str, float]]:
    """
    Compute normalized forward sensitivity indices of the endemic equilibrium
    state variables to each parameter using the implicit function theorem (Az=b).

    State variables: (e_h, i_h, r_h, N_h, e_v, i_v, N_v)
    Returns a dict of dicts: indices[state_var][param_name] = sensitivity index
    """
    state_names = ["e_h", "i_h", "r_h", "N_h", "e_v", "i_v", "N_v"]
    param_names = [f.name for f in fields(p)]

    # Build Jacobian A = dg/dx at the endemic equilibrium
    n = len(y_ee)
    A = np.zeros((n, n))
    h_vec = rel_step * np.abs(y_ee)
    h_vec[h_vec < 1e-12] = rel_step

    for i in range(n):
        y_plus = y_ee.copy(); y_plus[i] += h_vec[i]
        y_minus = y_ee.copy(); y_minus[i] -= h_vec[i]
        f_plus  = np.array(equilibrium_equations(y_plus,  p))
        f_minus = np.array(equilibrium_equations(y_minus, p))
        A[:, i] = (f_plus - f_minus) / (2.0 * h_vec[i])

    # For each parameter, solve Az = -dg/dp
    all_indices = {s: {} for s in state_names}

    for pname in param_names:
        q = getattr(p, pname)
        if q == 0.0:
            for s in state_names:
                all_indices[s][pname] = 0.0
            continue

        h = rel_step * abs(q)
        p_plus  = Params(**{**asdict(p), pname: q + h})
        p_minus = Params(**{**asdict(p), pname: q - h})

        g_plus  = np.array(equilibrium_equations(y_ee, p_plus))
        g_minus = np.array(equilibrium_equations(y_ee, p_minus))
        dg_dp = (g_plus - g_minus) / (2.0 * h)
        ...
        for i, sname in enumerate(state_names):
            xi = y_ee[i]
            if abs(xi) < 1e-12:
                all_indices[sname][pname] = 0.0
            else:
                all_indices[sname][pname] = z[i] * q / xi

    return all_indices


def get_endemic_equilibrium(p: Params, ic, t_end=200_000) -> np.ndarray:
    """Get the endemic equilibrium by long-time integration."""
...
def plot_sensitivity_ih(p_low, p_high, y_ee_low, y_ee_high, fname):
    """Plot sensitivity indices of i_h at the endemic equilibrium, split into
    two subplots (low and high transmission), ranked separately."""
    idx_low  = sensitivity_indices_xee(p_low,  y_ee_low)["i_h"]
    idx_high = sensitivity_indices_xee(p_high, y_ee_high)["i_h"]

    # Rank each separately by absolute sensitivity
    keys_low  = sorted(idx_low.keys(),  key=lambda k: abs(idx_low[k]),  reverse=True)
    keys_high = sorted(idx_high.keys(), key=lambda k: abs(idx_high[k]), reverse=True)

    labels_low  = [SYMBOL[k] for k in keys_low]
    labels_high = [SYMBOL[k] for k in keys_high]
    vals_low    = [idx_low[k]  for k in keys_low]
    vals_high   = [idx_high[k] for k in keys_high]

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    # --- Low transmission ---
    x_low = np.arange(len(keys_low))
    bars_low = ax1.bar(x_low, vals_low, color="C0", width=0.6)
    ax1.axhline(0, color="k", lw=0.6)
    ax1.set_xticks(x_low)
    ax1.set_xticklabels(labels_low, rotation=0)
    ax1.set_ylabel(r"Sensitivity index  $\Upsilon^{i_h}_{q}$")
    ax1.set_title(r"Low Transmission: Sensitivity of $i_h$")
    ax1.grid(axis="y", alpha=0.3)

    # highlight top 3 bars with a different color
    for bar in bars_low[:3]:
        bar.set_color("C0")

    # --- High transmission ---
    x_high = np.arange(len(keys_high))
    bars_high = ax2.bar(x_high, vals_high, color="C1", width=0.6)
    ax2.axhline(0, color="k", lw=0.6)
    ax2.set_xticks(x_high)
    ax2.set_xticklabels(labels_high, rotation=0)
    ax2.set_ylabel(r"Sensitivity index  $\Upsilon^{i_h}_{q}$")
    ax2.set_title(r"High Transmission: Sensitivity of $i_h$")
    ax2.grid(axis="y", alpha=0.3)

   # highlight top 3 bars with a different color
    for bar in bars_high[:3]:
        bar.set_color("C1")

    fig.suptitle(
        r"Sensitivity of infectious human proportion $i_h$ at endemic equilibrium",
        fontsize=13,
    )
    fig.tight_layout()
    fig.savefig(fname, dpi=130)
...
def main():
    ...
    # 7.5 Sensitivity of i_h at endemic equilibrium (Tables 5 & 6).
    print("=" * 72)
    print("STEP 5 — Sensitivity indices of i_h at endemic equilibrium")
    print("=" * 72)
    print("  Computing endemic equilibria (this may take a moment)...")
    y_ee_low  = get_endemic_equilibrium(LOW,  ic_low)
    y_ee_high = get_endemic_equilibrium(HIGH, ic_high)
    print(f"  Low  endemic eq:  i_h = {y_ee_low[1]:.4f}  (paper: 0.080)")
    print(f"  High endemic eq:  i_h = {y_ee_high[1]:.4f}  (paper: 0.16)")
    plot_sensitivity_ih(
        LOW, HIGH, y_ee_low, y_ee_high,
        "pics/sensitivity_ih.png"
    )
    print()
    print("Done.")
